...
exports.list = function (req, res, next) {
  ...
  function accessTemplate() {
    ...
    if (permission === '00000') {
      ...
      res.locals.flash = {
        ...
        message: 'У Вас нет прав доступа к шаблону "block".'
      };

      yesPage = false;

      res.render('template/block/body',
        {
          layout: 'admin',
          sidebar: sidebar,
          yesPage: yesPage,
    ...
    if (urlParsed.query.edit) {
      ...
      if (permission.indexOf('1', 3) === 3) {
        ...
        req.session.flash = {
          type: 'danger',
          intro: 'Ошибка доступа!',
          message: "У Вас нет прав на создание коллекции разделов!"
        };
        res.redirect(303, pathname);
      }
  ...
  function accessUrl() {
    ...
    if (urlParsed.query.dropStr) {
      ...
      Block.accessBlockandsectionID(urlParsed.query.dropStr, function (err, result) {
        if (err) return next(err);
      ...
      Block.accessBlockID(urlParsed.query.edit, function (err, result) {
      ...
      Block.accessBlockID(urlParsed.query.drop, function (err, result) {
      ...
      Block.accessBlockID(urlParsed.query.createSections, function (err, result) {
  ...
  function createSections() {
    ...
    if (urlParsed.query.createSections) {
      ...
      if (permission.indexOf('0', 4) === 4) {
        ...
        let block_id = Number(urlParsed.query.createSections);

        Block.getBlockIDAuthor(block_id, id_user, function (err, resultForm) {
          if (err) return next(err);

          if (resultForm.rowCount > 0) {

            Block.blockAndSectionSelect(block_id, id_user, function (err, result) {
              if (err) return next(err);

              sectionsSelect(result, function (err, sections) {
                if (err) return next(err);

                Block.getTableIdAuthor(block_id, id_user, function (err, resultTable) {
                  if (err) return next(err);

                  res.render('template/block/body-edit', {
                    layout: 'admin',
                    urlPage: req.url,
                    ...
                    sidebar: sidebar,
                    template: temp,
                    blockTitle: resultForm.rows[0].title,
                    blockAlias: resultForm.rows[0].alias,
                    sections: sections,
                    block: Block.tableBlockAndSection(resultTable, permission),
                    administrator: administrator
                  });
                })
              })
      ...
      if (permission.indexOf('1', 4) === 4) {
        ...
        Block.getBlockID(block_id, function (err, resultForm) {
          if (err) return next(err);

          if (resultForm.rowCount > 0) {
            Block.blockAndSectionSelectAll(block_id, function (err, result) {
              if (err) return next(err);
              ...
              sectionsSelect(result, function (err, sections) {
                ...
                Block.getTableId(block_id, function (err, resultTable) {
                  if (err) return next(err);
  ...
  function deleteStr() {
    ...
    if (urlParsed.query.dropStr) {
      ...
      let deleteStr = new Block({
        id: urlParsed.query.dropStr,
        author: id_user
      });
      deleteStr.deleteStrBlockandsection(function (err, result) {
        if (err) return next(err);
  ...
  function listRender() {
    ...
    blockList.list(function (err, result) {
      ...
      resultList = Block.tableListBlock(result, permission);
      ...
      //Просматривать, Публиковаь(0) | Удалять(1) | Править, редактировать(2) | Сохранять, добавлять(3) | Редактировать всех(4)
  ...
  let tasks = [getSection, initialization, accessValue, userMenu, accessTemplate, getAccess, accessUrl, createSections, listEdit, listDrop, deleteStr, publication, listRender];
...
exports.submit = function (req, res, next) {
  ...
  function submitCreate() {
      ...
      save.isset(function (err, result) {
        ...
        if (result === 0) {
          ...
          save.save(function (err, result) {
            ...
            if (result.rowCount === 1 && permission.indexOf('0', 0) === 0) {
    ...
    } else if (value.createSections && urlParsed.query.createSections) {

      if (value.selectSections) {

        let blockandsection = new Block(
          {
        ...
        req.session.flash = {
          type: 'danger',
          intro: 'Ошибка записи!',
          message: "Нет разделов для набора!"
        };
        res.redirect(303, 'back');
      }
  ...
  function submitDrop() {
    ...
    if (urlParsed.query.drop && value.drop && permission.indexOf('1', 1) === 1) {
      ...
      drop.dropBlock(function (err, result) {
        if (err) return next(err);
...
function sectionsSelect(sections, fn) {
    ...
    co(function* () {
      ...
      for (let i = 0; i < sections.length; i++) {
        ...
        email = yield new Promise(function (resolve) {
          ...
          Block.getEmailAuthor(sections[i].author, function (err, result) {
        ...
        str += '<option value="' + sections[i].id + '"><b>' + sections[i].title + '</b> [' + sections[i].alias + ']' + ' [' + email + ']' + '</option>' + '\n';
      }
