...
exports.list = function (req, res, next) {
  ...
  function accessTemplate() {
    ...
    if (permission === '00000') {
      ...
      res.locals.flash = {
        ...
        message: 'У Вас нет прав доступа к шаблону "price-cottages".'
      };

      yesPage = false;

      res.render('template/price-cottages/body',
        {
          layout: 'admin',
          yesPage: yesPage,
  ...
  function columnPrice() {
    ...
    Price.colPrice(req.session.uid, function (err, result) {
      ...
      if (result.rowCount > 0) {
        ...
        code = result.rows[0].code_price_cottages;
        ...
        if (code.indexOf('1', 8) === 8) {
          col.material = 'checked';
        }

        if (code.indexOf('1', 9) === 9) {
          col.category_land = 'checked';
        }
  ...
  function listRender() {
    ...
    res.render('template/price-cottages/body',
      {
        layout: 'admin',
        title: nameSection + '.  <button class="btn btn-primary btn-sm" data-toggle="modal" data-target="#myModal">Настроить вывод колонок</button> <button form="csvForm" type="submit" class="btn btn-success btn-sm" name="price-cottages[csv]" value="1">Скачать таблицу в Excel</button> <a href="/admin/template/cottages" class="btn btn-warning btn-sm" role="button">Выйти из прайса</a>',
        permit: permitForm,
        permission: permission,
        table: resultList,
        back: '/admin/template/price-cottages',
...
exports.submit = function (req, res, next) {
  ...
  function stringSQL() {
    ...
    if (value.button === '1') {
      ...
      let code = '';
      let arrPrice = ['type', 'section', 'city', 'street', 'price', 'area_house', 'area_land', 'storey', 'material', 'category_land', 'kdn', 'note', 'agent', 'photo'];
  ...
  function getPrice() {
    ...
    if (value.csv === '1') {
      ...
      co(function*() {
        ...
        let str = 'attachment; filename = ' + date + '_cottages.xlsx';
