...
exports.list = function (req, res, next) {
  ...
  function getSection() {
    ...
    Permit.getSection('/admin/template/commercial', function (err, result) {
      ...
      pool.connect(function (err, client, done) {
        ...
        workbook.SheetNames.forEach(function (sheetName) {
          ...
          for (let i = 0; i < roa.length; i++) {
            ...
            if (sheetName === 'Гаражи-офисы-пр.базы-магазины') {

              roa[i]['раздел'] = sheetName;

              client.query('INSERT INTO parser_commercial (note, price, tel, section) VALUES($1, $2, $3, $4)', [roa[i]["гаражи / офисы /пр. базы /магазины /пр.помещения"], roa[i]["цена"], roa[i]["телефоны агенства, риэлтора"], roa[i]["раздел"]], function (err, result) {
                done();
                if (err) return next(err);
        ...
        Parser.getAllCity(function (err, result) {
          ...
          Parser.getMainCity('Нижнекамск', permission, id_agency, id_moderator_agency, id_user, function (err, result) {
            ...
            if (result.rowCount > 0) {
              ...
              if (result.rowCount > 0) {
                city += '\t<li><a href="/admin/administrator/parser-commercial?editCity=' + result.rows[0].id_city + '">[' + result.rows[0].sum + '] ' + '<b>' + result.rows[0].title + '</b> - ' + result.rows[0].region + '</a></li>\n';
              }
              ...
              Parser.listCity('Нижнекамск', permission, id_agency, id_moderator_agency, id_user, function (err, result) {
                ...
                if (result.rowCount > 0) {
                  ...
                  for (let i = 0; i < cityList.length; i++) {
                    ...
                    city += '\t<li><a href="/admin/administrator/parser-commercial?editCity=' + cityList[i].id_city + '">[' + result.rows[i].sum + '] ' + '<b>' + cityList[i].title + '</b> - ' + cityList[i].region + '</a></li>\n';
                  }

                  for (let key in objCity) {
                    city += '\t<li><a href="/admin/administrator/parser-commercial?editCity=' + key + '"><b>' + objCity[key] + '</b> - ' + objRegion[key] + '</a></li>\n';
                  }
  ...
  function listRender() {
    ...
    let nameTemplate = 'Коммерческая недвижимость. Парсер XLS. ';
    ...
    res.render('administrator/parser-commercial/body', {
  ...
  let tasks = [accessAdministrator, getSection, accessValue, userMenu, editCity, listParser, listEdit, type, section, listSelectAgents, streetSelect, price, listCity, listTable, listRender];
