...
ParserCottage.list = function (fn) {
  ...
  pool.connect( function (err, client, done) {
    ...
    client.query('SELECT id as "Редактирование", note AS "Коммерческая недвижимость", price AS цена, tel AS "телефоны риэлтора", section AS "раздел" FROM parser_commercial ORDER BY id', function (err, result) {
      done();
...
ParserCottage.listLimit = function (limit, offset, fn) {
  ...
  pool.connect( function (err, client, done) {
    ...
    client.query('SELECT id as "Редактирование", note AS "Коммерческая недвижимость", price AS цена, tel AS "телефоны риэлтора", section' +
      ' AS "раздел" FROM parser_commercial ORDER BY id LIMIT $1 OFFSET $2', [limit, offset], function (err, result) {
      done();
      if (err) return fn(err, null);
    ...
    for (let j = 0; j < result.rows.length; j++) {
      ...
      for (let i = 0; i < result.fields.length; i++) {
        ...
        if (result.fields[i].name == 'Редактирование') {
          ...
          str += '<a class="btn btn-primary btn-xs btn-margins" role="button" href="/admin/administrator/parser-commercial?edit=' + row[cols] + strPath + '">править</a>'
...
ParserCottage.getOneRecord = function (id, fn) {
  pool.connect( function (err, client, done) {
    ...
    client.query('SELECT * FROM parser_commercial WHERE id = $1', [id], function (err, result) {
      done();
      if (err) return fn(err, null);
...
ParserCottage.getMainCity = function(mainCity, permission, id_agency, id_moderator_agency, id_user, fn){
  ...
  if(permission.indexOf('1', 4) === 4){
    ...
    pool.connect(function (err, client, done) {
      ...
      client.query("SELECT DISTINCT c.id_city, (SELECT title FROM city WHERE id_city = c.id_city), (SELECT (SELECT title FROM region WHERE id_region = region_id) FROM city WHERE id_city = c.id_city ) AS region, count(*) OVER (PARTITION BY c.title) AS sum FROM commercial a LEFT JOIN street s ON (a.street = s.id_street) LEFT JOIN city c ON (s.city_id = c.id_city) WHERE c.title = $1", [mainCity], function (err, result) {
        done();
...
ParserCottage.prototype.save = function (fn) {
  ...
  (async () => {
    ...
    try {
      ...
      let result1 = await client.query('INSERT INTO commercial (type, agent, street, price, area_house, note, node_id) VALUES ($1, $2, $3, $4, $5, $6, $7)', [ type, agent, street, price, area_house, note, id ]);

      let result2 = await client.query('UPDATE node SET alias = $1 WHERE id = $2', [ id, id ]);

      let result3 = await client.query('DELETE FROM parser_commercial WHERE id = $1', [ id_parser ]);
